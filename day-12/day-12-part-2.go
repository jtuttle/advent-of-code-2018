...
func main() {
	...
	for i := 1; i <= generations; i++ {
		sumBefore := computePlantPotSum(state)
		
		state = nextState(state, rules)

		sumDelta := computePlantPotSum(state) - sumBefore

		if prevSumDelta == sumDelta {
			genConverge = i
			break
		}
	...
	fmt.Println("Converged to", prevSumDelta, "on generation", genConverge)

	currentSum := computePlantPotSum(state)

	genRemaining := generations - genConverge
	futureSum := prevSumDelta * genRemaining

	fmt.Println("Plant pot sum:", currentSum + futureSum)
}
