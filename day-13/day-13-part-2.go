...
func removeCrashedCarts(carts []*Cart) []*Cart {
	safeCarts := make([]*Cart, 0)

	for _, cart := range carts {
		safe := true
		
		for _, otherCart := range carts {
			if cart != otherCart && cart.position == otherCart.position {
				safe = false
			}
		}

		if safe {
			safeCarts = append(safeCarts, cart)
		}
	}
...
func cartHasCrashed(cart *Cart, carts []*Cart) bool {
	for _, otherCart := range carts {
		if otherCart != cart && otherCart.position == cart.position {
			return true
		}
	}
...
func main() {
	...
	for len(carts) > 1 {
		for _, cart := range carts {
			if !cartHasCrashed(cart, carts) {
				moveCart(tracks, cart)
			}
		}

		carts = removeCrashedCarts(carts)
	}

	//printState(tracks, carts)
	
	fmt.Println("Last cart position:", &carts[0].position)
}
